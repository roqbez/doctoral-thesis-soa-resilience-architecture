...
package br.ufsc.gsigma.services.execution.bpel.ode.scheduler;
 ...
 *
 * The challenge is to achieve high performance in a small memory footprint without loss of reliability while supporting distributed/clustered configurations.
 *
 * The design is based around three time horizons: "immediate", "near future", and "everything else". Immediate jobs (i.e. jobs that are about to be up) are written to the database and kept in an in-memory priority queue. When they execute, they are
 * removed from the database. Near future jobs are placed in the database and assigned to the current node, however they are not stored in memory. Periodically jobs are "upgraded" from near-future to immediate status, at which point they get loaded
 * into memory. Jobs that are further out in time, are placed in the database without a node identifer; when they are ready to be "upgraded" to near-future jobs they are assigned to one of the known live nodes. Recovery is rather straighforward, with
 * stale node identifiers being reassigned to known good nodes.
 *
...
public class SimpleScheduler implements Scheduler, TaskRunner {
	...
	protected static final Log __log = LogFactory.getLog(SimpleScheduler.class);

	@SuppressWarnings("unused")
	private static final int DEFAULT_TRANSACTION_TIMEOUT = 60 * 1000;

	/**
	 * Jobs scheduled with a time that is between [now, now+immediateInterval] will be assigned to the current node, and placed directly on the todo queue.
	 */
	long _immediateInterval = 30000;

	/**
	 * Jobs scheduled with a time that is between (now+immediateInterval,now+nearFutureInterval) will be assigned to the current node, but will not be placed on the todo queue (the promoter will pick them up).
	 */
	...
	/**
	 * Estimated sustained transaction per second capacity of the system. e.g. 100 means the system can process 100 jobs per seconds, on average This number is used to determine how many jobs to load from the database at once.
	 */
	int _tps = 100;
	...
	protected volatile JobProcessor _jobProcessor;

	protected volatile JobProcessor _polledRunnableProcessor;
	...
	/**
	 * Set of outstanding jobs, i.e., jobs that have been enqueued but not dequeued or dispatched yet. Used to avoid cases where a job would be dispatched twice if the server is under high load and does not fully process a job before it is reloaded
	 * from the database.
	 */
	private ConcurrentHashMap<String, Long> _outstandingJobs = new ConcurrentHashMap<String, Long>();
	/**
	 * Set of Jobs processed since the last LoadImmediate task. This prevents a race condition where a job is processed twice. This could happen if a LoadImediate tasks loads a job from the db before the job is processed but puts it in the
	 * _outstandingJobs map after the job was processed . In such a case the job is no longer in the _outstandingJobs map, and so it's queued again.
	 */
		...
		return _exec.submit(new Callable<T>() {
			public T call() throws Exception {
				try {
					...
					__log.error("An exception occured while executing an isolated transaction, " + "the transaction is going to be abandoned.", e);
					return null;
				}
			}
		});
			...
			do {
				try {
					if (__log.isTraceEnabled())
						__log.trace("Beginning a new transaction");
					txm.begin();
				} catch (Exception e) {
				...
				try {
					...
					if (ex != null && immediateRetryCount > 0) {
						if (__log.isDebugEnabled())
							__log.debug("Will retry the transaction in " + _immediateTransactionRetryInterval + " msecs on " + _txm + " for error: ", ex);
	...
	public String schedulePersistedJob(final JobDetails jobDetail, Date when) throws ContextException {
		 ...
		 return scheduleVolatileJob(true, jobDetail, when);
...
//
//		return schedulePersistedJob(createJob(when, jobDetail), when, ctime);
	}
	...
	public String scheduleMapSerializableRunnable(MapSerializableRunnable runnable, Date when) throws ContextException {
		...
		return schedulePersistedJob(createJob(when, jobDetails), when, ctime);
	}

	protected Job createJob(Date when, JobDetails jobDetails) {
		return new Job(when.getTime(), true, jobDetails);
	}
	...
	public String scheduleVolatileJob(boolean transacted, JobDetails jobDetail, Date when) throws ContextException {
		...
		if (when == null)
			when = new Date(ctime);
		Job job = createJob(transacted, jobDetail, when);
		job.persisted = false;
		addTodoOnCommit(job);
		return job.toString();
	}
	...
	protected Job createJob(boolean transacted, JobDetails jobDetail, Date when) {
		return new Job(when.getTime(), transacted, jobDetail);
	}
	...
	public class RunJob implements Callable<Void> {
		...
		public RunJob(Job job, JobProcessor processor) {
			this.job = job;
	...
	protected void runJob(final Job job) {
		_exec.submit(createRunJob(job, _jobProcessor));
	}
	 ...
	 * <ul>
	 * <li>1. The runnable is being scheduled; the poller persistent job dispatches the runnable to a runnable delegate thread and schedules itself to a later time.</li>
	 * <li>2. The runnable is running; the poller job re-schedules itself every time it sees the runnable is not completed.</li>
	 * <li>3. The runnable failed; the poller job passes the exception thrown on the runnable down, and the standard scheduler retries happen.</li>
	 * <li>4. The runnable completes; the poller persistent does not re-schedule itself.</li>
	 * <li>5. System powered off and restarts; the poller job does not know what the status of the runnable. This is handled just like the case #1.</li>
	 * </ul>
	 * <p/>
	...
	protected void runPolledRunnable(final Job job) {
		_exec.submit(createRunJob(job, _polledRunnableProcessor));
	}

	protected RunJob createRunJob(final Job job, JobProcessor processor) {
	...
	boolean doLoadImmediate() {
		...
		// if (!InfinispanDatabase.getInstance().isLeader()) {
		// return true;
		// }
		...
		try {
			...
			__log.trace("LOAD IMMEDIATE started");
			...
			try {
				...
				if (batch <= 0) {
					if (__log.isDebugEnabled())
						__log.debug("Max capacity reached: " + _outstandingJobs.size() + " jobs dispacthed i.e. queued or being executed");
					return true;
				}

				if (__log.isTraceEnabled())
					__log.trace("loading " + batch + " jobs from db");
				...
				if (__log.isTraceEnabled())
					__log.trace("loaded " + jobs.size() + " jobs from db");
				...
				__log.trace("LOAD IMMEDIATE complete");
			}

		} finally {
			TxLog.restoreLogging();
		}
	}
		...
		if (_processedSinceLastLoadTask.get(job.jobId) == null) {
			if (_outstandingJobs.putIfAbsent(job.jobId, job.schedDate) == null) {
				if (job.schedDate <= System.currentTimeMillis()) {
					...
					if (__log.isDebugEnabled()) {
						__log.debug("Scheduling job " + job.jobId + " to run NOW");
					}
					...
					if (__log.isDebugEnabled()) {
						__log.debug("Putting job " + job.jobId + " in TODO queue");
					}
				...
				if (__log.isDebugEnabled())
					__log.debug("Job " + job.jobId + " is being processed (outstanding job)");
			}
		} else {
			if (__log.isDebugEnabled())
				__log.debug("Job " + job.jobId + " is being processed (processed since last load)");
		}
	...
	private class CheckStaleNodes extends SchedulerTask {
		...
		public void run() {
			...
			__log.trace("CHECK STALE NODES started");
			for (String nodeId : _knownNodes) {
				Long lastSeen = _lastHeartBeat.get(nodeId);
				if ((lastSeen == null || (System.currentTimeMillis() - lastSeen) > _staleInterval) && !_nodeId.equals(nodeId)) {
