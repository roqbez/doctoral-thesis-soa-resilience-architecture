...
/**
 * AXIS2 implementation of the {@link org.apache.ode.bpel.iapi.BindingContext} interface. Deals with the activation of endpoints.
 *
...
public class BindingContextImpl implements BindingContext {
	...
	protected AxisODEServer _server;
	...
	public BindingContextImpl(AxisODEServer server) {
		_server = server;
	}
	...
	public EndpointReference activateMyRoleEndpoint(QName processId, Endpoint myRoleEndpoint) {
		try {
			...
			throw new ContextException("Could not activate endpoint for service " + myRoleEndpoint.serviceName + " and port " + myRoleEndpoint.portName, axisFault);
		}
	}
	...
	protected ODEService createService(ProcessConf pconf, QName serviceName, String portName) throws AxisFault {
		...
		ODEService odeService = createODEService(pconf, serviceName, portName, axisService);
	...
	protected ODEService createODEService(ProcessConf pconf, QName serviceName, String portName, AxisService axisService) throws AxisFault {
		return new ODEService(axisService, pconf, serviceName, portName, _server._bpelServer, _server._txMgr);
	}
	...
	/**
	 * /!\ Monkey patching to remove references to the service: Manually & externally & really really horribly fix for ODE-580/AXIS2-3870 The exception handling is for locked down environment where
	 * reflection would not be allowed...
	 *
		...
		try {
			if (WsdlUtils.useHTTPBinding(def, serviceName, portName)) {
				if (__log.isDebugEnabled())
					__log.debug("Creating HTTP-bound external service " + serviceName);
				...
				if (__log.isDebugEnabled())
					__log.debug("Creating SOAP-bound external service " + serviceName);
				extService = new SoapExternalService(pconf, serviceName, portName, _server._executorService, _server._axisConfig, _server._scheduler, _server._bpelServer,
