...
public class DeploymentWebService {
	...
	public void enableService(AxisConfiguration axisConfig, ProcessStore store, DeploymentPoller poller, String rootpath, String workPath) throws AxisFault, WSDLException {
		...
		AxisService deployService = ODEAxisService.createService(axisConfig, new QName("http://www.apache.org/ode/deployapi", "DeploymentService"), "DeploymentPort", "DeploymentService", def,
				new DeploymentMessageReceiver());
				...
				if (operation.equals("deploy")) {
					...
					if (namePart == null) {
						...
						if (namePart == null) {
							...
							__log.warn("Invalid incoming request detected for operation " + messageContext.getAxisOperation().getName() + ". Name part should have no namespace but has "
					...
					if (packagePart == null) {
						...
						if (packagePart != null && __log.isWarnEnabled()) {
							__log.warn("Invalid incoming request detected for operation " + messageContext.getAxisOperation().getName() + ". Package part should have no namespace but has "
					...
					if (packagePart != null) {
						...
						if (zip == null) {
							...
							if (zip != null && __log.isWarnEnabled()) {
								...
								__log.warn("Invalid incoming request detected for operation " + messageContext.getAxisOperation().getName() + ". <zip/> element namespace should be "
										+ Namespaces.ODE_DEPLOYAPI_NS + " but was " + ns);
							}
						}
					}
					...
					if (part == null)
						throw new OdeFault("Missing bundle name in undeploy message.");
