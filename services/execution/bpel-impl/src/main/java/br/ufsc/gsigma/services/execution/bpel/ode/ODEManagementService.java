...
public class ODEManagementService {

	private static final Log __log = LogFactory.getLog(ODEManagementService.class);
	...
	public void enableService(AxisConfiguration axisConfig, BpelServer server, ProcessStore _store, String rootpath) {
		ProcessAndInstanceManagementImpl pm = createProcessAndInstanceManagement(server, _store);
		_processMgmt = pm;
		_instanceMgmt = pm;
		...
		try {
			...
			AxisService processService = ODEAxisService.createService(axisConfig, PM_SERVICE_NAME, PM_PORT_NAME, PM_AXIS2_NAME, def, createDynamicMessageReceiver(_processMgmt));
			AxisService instanceService = ODEAxisService.createService(axisConfig, IM_SERVICE_NAME, IM_PORT_NAME, IM_AXIS2_NAME, def, createDynamicMessageReceiver(_instanceMgmt));
	...
	protected ProcessAndInstanceManagementImpl createProcessAndInstanceManagement(BpelServer server, ProcessStore _store) {
		return new ProcessAndInstanceManagementImpl(server, _store);
	}
	...
	public static class DynamicMessageReceiver<T> extends AbstractMessageReceiver {
		T _service;
		...
		public void invokeBusinessLogic(MessageContext messageContext) throws AxisFault {
			...
			OMElement response;
			try {
				response = service.invoke(messageContext.getAxisOperation().getName().getLocalPart(), messageContext.getEnvelope().getBody().getFirstElement());
				if (response != null) {
