...
public class SoapExternalService implements ExternalService {
	...
	protected static final Log __log = LogFactory.getLog(SoapExternalService.class);
	...
	public SoapExternalService(ProcessConf pconf, QName serviceName, String portName, ExecutorService executorService, AxisConfiguration axisConfig, Scheduler sched, BpelServer server, MultiThreadedHttpConnectionManager connManager,
			ClusterUrlTransformer clusterUrlTransformer) throws AxisFault {
		this(pconf, pconf.getDefinitionForService(serviceName), serviceName, portName, executorService, axisConfig, sched, server, connManager, clusterUrlTransformer);
	}

	@SuppressWarnings({ "rawtypes" })
	public SoapExternalService(ProcessConf pconf, Definition definition, QName serviceName, String portName, ExecutorService executorService, AxisConfiguration axisConfig, Scheduler sched, BpelServer server,
			MultiThreadedHttpConnectionManager connManager, ClusterUrlTransformer clusterUrlTransformer) throws AxisFault {
		...
		_sched = sched;
		_converter = createSoapMessageConverter(server, _definition, serviceName, portName);
		_server = server;
		_pconf = pconf;
	...
	protected SoapMessageConverter createSoapMessageConverter(BpelServer server, Definition definition, QName serviceName, String portName) throws AxisFault {
		return new SoapMessageConverter(definition, serviceName, portName);
	}
	...
	public void invoke(final PartnerRoleMessageExchange odeMex) {
		...
		try {
			...
			/*
			 * make the given options the parent so it becomes the defaults of the MessageContexgt. That allows the user to override specific options on a given message context and not affect the
			 * overall options.
			 */
			...
			if (endpointUrl.equals(mexEndpointUrl)) {
				...
				if (address != null) {
					if (__log.isTraceEnabled())
						__log.trace("Endpoint URL overridden by property files. " + mexEndpointUrl + " => " + address);
					axisEPR.setAddress(address);
				}
			} else {
				if (__log.isTraceEnabled())
					__log.trace("Endpoint URL overridden by process. " + endpointUrl + " => " + mexEndpointUrl);
			}

			axisEPR.setAddress(_clusterUrlTransformer.rewriteOutgoingClusterURL(axisEPR.getAddress()));

			if (__log.isDebugEnabled()) {
				__log.debug("Sending message to " + axisEPR.getAddress() + " using MEX " + odeMex //
						+ "\n\tsentMessage=" + soapEnv.getBody());
			}
			...
			if (isTwoWay) {
				invokeTwoWay(odeMex, mctx, operationClient);

			} else { /** one-way case * */
				invokeOneWay(odeMex, mctx, operationClient);
			}

			mctx.getEnvelope().getBody();
	...
	protected void invokeTwoWay(final PartnerRoleMessageExchange odeMex, final MessageContext mctx, final OperationClient operationClient) {
	...
	private ServiceClient getServiceClient() throws AxisFault {
		...
		if (client == null || !client.getAxisService().getName().equals(anonymousService.getName())) {
			...
			synchronized (_axisConfig) {
				...
				if (client != null) {
					if (__log.isTraceEnabled())
						__log.trace("Clean up and discard ServiceClient");
					client.cleanup();
				}
				if (__log.isTraceEnabled())
					__log.trace("Create a new ServiceClient for " + anonymousService.getName());
	...
	/**
	 * Extracts the action to be used for the given operation. It first checks to see if a value is specified using WS-Addressing in the portType, it then falls back onto getting it from the SOAP
	 * Binding.
	 *
	 * @param operation
	 *            the name of the operation to get the Action for
	...
	private void writeHeader(MessageContext ctxt, PartnerRoleMessageExchange odeMex) {
		...
		if (myRoleWSAEPR != null) {
			...
			Element address = DOMUtils.findChildByName(serviceElement, new QName(Namespaces.WS_ADDRESSING_NS, "Address"), true);
			if (__log.isDebugEnabled()) {
			...
			__log.trace("My-Role EPR not specified, SEP will not be used.");
		}
					...
					try {
						...
						if (isFault) {
							...
							if (fault != null) {
								...
								odeMex.replyWithFailure(FailureType.OTHER, reply.getEnvelope().getBody().getFault().getText(), OMUtils.toDOM(reply.getEnvelope().getBody()));
							}
						} else {
	...
	/**
	 * This class wraps a {@link org.apache.axis2.client.ServiceClient} and watches changes (deletions,creations,updates) on a Axis2 service config file named {service-name}.axis2.
	 * <p/>
	 * The {@link org.apache.axis2.client.ServiceClient} instance is created from the main Axis2 config instance and this service-specific config file.
	 */
