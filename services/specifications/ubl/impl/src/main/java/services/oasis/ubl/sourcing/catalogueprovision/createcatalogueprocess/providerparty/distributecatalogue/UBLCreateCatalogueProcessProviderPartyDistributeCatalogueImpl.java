package services.oasis.ubl.sourcing.catalogueprovision.createcatalogueprocess.providerparty.distributecatalogue;
...
public class UBLCreateCatalogueProcessProviderPartyDistributeCatalogueImpl extends AbstractUBLService implements UBLCreateCatalogueProcessProviderPartyDistributeCatalogue {

	private final Logger logger = Logger.getLogger(getClass());

	private boolean interactive;

	public UBLCreateCatalogueProcessProviderPartyDistributeCatalogueImpl(boolean interactive) {
		this.interactive = interactive;
	}

	@Override
	public CatalogueType distributeCatalogue(CatalogueType input) {
		logRequest("distributeCatalogue", null, input);
		return input;
	}

	@Override
	public void distributeCatalogueAsync(ProcessContext processContext, CatalogueType input) {
		logRequest("distributeCatalogueAsync", processContext, input);

		DistributeCatalogueAsyncCallback callback = new DistributeCatalogueAsyncCallback();
		callback.processContext = processContext;
		callback.output = input;

		if (interactive)
			UIUtil.createQuestionUI("ProviderParty/distributeCatalogueAsync " + "[processId=" + processContext.getProcessId() + "]", "Choose OK to continue the process", new Object[] { input },
					new String[] { "OK" });
		else
