...
public class UBLOrderingProcessBuyerPartyAcceptOrderImpl extends AbstractUBLService implements UBLOrderingProcessBuyerPartyAcceptOrder {

	private final Logger logger = Logger.getLogger(getClass());

	private boolean interactive;

	public UBLOrderingProcessBuyerPartyAcceptOrderImpl(boolean interactive) {
		this.interactive = interactive;
	}

	@Override
	public String acceptOrder(OrderResponseSimpleType inputOrderResponseSimple, OrderResponseType inputOrderResponse) {
		logRequest("acceptOrder", null, inputOrderResponseSimple, inputOrderResponse);
		return "accepted";
	}

	@Override
	public void acceptOrderAsync(ProcessContext processContext, OrderResponseSimpleType inputOrderResponseSimple, OrderResponseType inputOrderResponse) {

		logRequest("acceptOrderAsync", processContext, inputOrderResponseSimple, inputOrderResponse);
		...
		if (interactive)
			UIUtil.createQuestionUI("BuyerParty/acceptOrderAsync " + "[processId=" + processContext.getProcessId() + "]", "Choose OK to continue the process", new Object[] { inputOrderResponseSimple, inputOrderResponse }, new String[] { "OK" });
		else
