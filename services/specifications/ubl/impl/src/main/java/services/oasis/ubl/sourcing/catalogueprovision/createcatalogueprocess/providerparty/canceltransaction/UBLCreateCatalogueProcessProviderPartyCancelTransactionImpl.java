package services.oasis.ubl.sourcing.catalogueprovision.createcatalogueprocess.providerparty.canceltransaction;
...
public class UBLCreateCatalogueProcessProviderPartyCancelTransactionImpl extends AbstractUBLService implements UBLCreateCatalogueProcessProviderPartyCancelTransaction {

	private final Logger logger = Logger.getLogger(getClass());

	private boolean interactive;

	public UBLCreateCatalogueProcessProviderPartyCancelTransactionImpl(boolean interactive) {
		this.interactive = interactive;
	}

	@Override
	public String cancelTransaction(CatalogueType input) {
		logRequest("cancelTransaction", null, input);
		return "accepted";
	}

	@Override
	public void cancelTransactionAsync(ProcessContext processContext, CatalogueType input) {
		logRequest("cancelTransactionAsync", processContext, input);

		CancelTransactionAsyncCallback callback = new CancelTransactionAsyncCallback();
		callback.processContext = processContext;
		callback.output = "accepted";

		if (interactive)
			UIUtil.createQuestionUI("ProviderParty/cancelTransactionAsync " + "[processId=" + processContext.getProcessId() + "]", "Choose OK to continue the process", new Object[] { input },
					new String[] { "OK" });
		else
