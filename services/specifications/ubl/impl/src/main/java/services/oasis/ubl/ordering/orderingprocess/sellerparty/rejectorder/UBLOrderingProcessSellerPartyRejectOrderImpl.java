...
public class UBLOrderingProcessSellerPartyRejectOrderImpl extends AbstractUBLService implements UBLOrderingProcessSellerPartyRejectOrder {

	private final Logger logger = Logger.getLogger(getClass());

	private boolean interactive;

	public UBLOrderingProcessSellerPartyRejectOrderImpl(boolean interactive) {
		this.interactive = interactive;
	}

	@Override
	public void rejectOrder(OrderType input, Holder<OrderResponseType> outputOrderResponse, Holder<OrderResponseSimpleType> outputOrderResponseSimple) {

		logRequest("rejectOrder", null, input);
	...
	@Override
	public void rejectOrderAsync(ProcessContext processContext, OrderType input) {

		logRequest("rejectOrderAsync", processContext, input);
		...
		if (interactive)
			UIUtil.createQuestionUI("SellerParty/rejectOrderAsync " + "[processId=" + processContext.getProcessId() + "]", "Choose OK to continue the process", new Object[] { input },
					new String[] { "OK" });
		else
