...
import com.github.dockerjava.api.command.LogContainerCmd.Exec;
...
public class CustomJerseyDockerCmdExecFactory implements DockerCmdExecFactory {
	...
	private X509HostnameVerifier getHostnameVerifier() {
		String hostNameVerifier = System.getProperty("HTTPClient.defaultHostnameVerifier");
		try {
			if (isBlank(hostNameVerifier) && !this.verifyHostname) {
				return SSLConnectionSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER;
			}
			Class<?> clazz = Class.forName(hostNameVerifier);
			Constructor<?> ctor = clazz.getConstructor();
			return (X509HostnameVerifier) ctor.newInstance((Object[]) null);
		} catch (Exception e) {
			LOGGER.error("Not able to load the class for host name verification through reflection.", e);
	...
	private org.apache.http.config.Registry<ConnectionSocketFactory> getSchemeRegistry(final URI originalUri, SSLContext sslContext) {
		...
		if (sslContext != null) {
			registryBuilder.register("https", new SSLConnectionSocketFactory(sslContext, getHostnameVerifier()));
		}
	...
	public CustomJerseyDockerCmdExecFactory withReadTimeout(Integer readTimeout) {
		this.readTimeout = readTimeout;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withConnectTimeout(Integer connectTimeout) {
		this.connectTimeout = connectTimeout;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withMaxTotalConnections(Integer maxTotalConnections) {
		this.maxTotalConnections = maxTotalConnections;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withMaxPerRouteConnections(Integer maxPerRouteConnections) {
		this.maxPerRouteConnections = maxPerRouteConnections;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withConnectionRequestTimeout(Integer connectionRequestTimeout) {
		this.connectionRequestTimeout = connectionRequestTimeout;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withClientResponseFilters(ClientResponseFilter... clientResponseFilter) {
		this.clientResponseFilters = clientResponseFilter;
		return this;
	}

	public CustomJerseyDockerCmdExecFactory withClientRequestFilters(ClientRequestFilter... clientRequestFilters) {
	...
	@Override
	public Exec createLogContainerCmdExec() {
		return new CustomLogContainerCmdExec(getBaseResource(), getDockerClientConfig());
	}

	private class CustomLogContainerCmdExec extends LogContainerCmdExec {

		// private Logger LOGGER = LoggerFactory.getLogger(LogContainerCmdExec.class);

		public CustomLogContainerCmdExec(WebTarget baseResource, DockerClientConfig dockerClientConfig) {
		...
		// }
		//
		// CustomLogContainerCmd customCmd = command instanceof CustomLogContainerCmd ? (CustomLogContainerCmd) command : null;
		//
		// if (customCmd != null && customCmd.getSinceTimestamp() != null) {
		// webTarget = webTarget.queryParam("since", customCmd.getSinceTimestamp());
		// } else if (command.getSince() != null) {
